...
(define (clip-line-x start-in-bounds? x x1 y1 z1 x2 y2 z2)
  (define t (/ (- x x1) (- x2 x1)))
  (if start-in-bounds?
      (values x1 y1 z1 x (+ y1 (* t (- y2 y1))) (+ z1 (* t (- z2 z1))))
      (values x (+ y1 (* t (- y2 y1))) (+ z1 (* t (- z2 z1))) x2 y2 z2)))

(define (clip-line-y start-in-bounds? y x1 y1 z1 x2 y2 z2)
  (define t (/ (- y y1) (- y2 y1)))
  (if start-in-bounds?
      (values x1 y1 z1 (+ x1 (* t (- x2 x1))) y (+ z1 (* t (- z2 z1))))
      (values (+ x1 (* t (- x2 x1))) y (+ z1 (* t (- z2 z1))) x2 y2 z2)))

(define (clip-line-z start-in-bounds? z x1 y1 z1 x2 y2 z2)
  (define t (/ (- z z1) (- z2 z1)))
  (if start-in-bounds?
      (values x1 y1 z1 (+ x1 (* t (- x2 x1))) (+ y1 (* t (- y2 y1))) z)
      (values (+ x1 (* t (- x2 x1))) (+ y1 (* t (- y2 y1))) z x2 y2 z2)))

(define (clip-line-x-min x-min x1 y1 z1 x2 y2 z2)
  (cond [(and (x1 . >= . x-min) (x2 . < . x-min))
         (clip-line-x #t x-min x1 y1 z1 x2 y2 z2)]
        [(and (x2 . >= . x-min) (x1 . < . x-min))
         (clip-line-x #f x-min x1 y1 z1 x2 y2 z2)]
        [else
         (values x1 y1 z1 x2 y2 z2)]))

(define (clip-line-x-max x-max x1 y1 z1 x2 y2 z2)
  (cond [(and (x1 . <= . x-max) (x2 . > . x-max))
         (clip-line-x #t x-max x1 y1 z1 x2 y2 z2)]
        [(and (x2 . <= . x-max) (x1 . > . x-max))
         (clip-line-x #f x-max x1 y1 z1 x2 y2 z2)]
        [else
         (values x1 y1 z1 x2 y2 z2)]))

(define (clip-line-y-min y-min x1 y1 z1 x2 y2 z2)
  (cond [(and (y1 . >= . y-min) (y2 . < . y-min))
         (clip-line-y #t y-min x1 y1 z1 x2 y2 z2)]
        [(and (y2 . >= . y-min) (y1 . < . y-min))
         (clip-line-y #f y-min x1 y1 z1 x2 y2 z2)]
        [else
         (values x1 y1 z1 x2 y2 z2)]))

(define (clip-line-y-max y-max x1 y1 z1 x2 y2 z2)
  (cond [(and (y1 . <= . y-max) (y2 . > . y-max))
         (clip-line-y #t y-max x1 y1 z1 x2 y2 z2)]
        [(and (y2 . <= . y-max) (y1 . > . y-max))
         (clip-line-y #f y-max x1 y1 z1 x2 y2 z2)]
        [else
         (values x1 y1 z1 x2 y2 z2)]))

(define (clip-line-z-min z-min x1 y1 z1 x2 y2 z2)
  (cond [(and (z1 . >= . z-min) (z2 . < . z-min))
         (clip-line-z #t z-min x1 y1 z1 x2 y2 z2)]
        [(and (z2 . >= . z-min) (z1 . < . z-min))
         (clip-line-z #f z-min x1 y1 z1 x2 y2 z2)]
        [else
         (values x1 y1 z1 x2 y2 z2)]))

(define (clip-line-z-max z-max x1 y1 z1 x2 y2 z2)
  (cond [(and (z1 . <= . z-max) (z2 . > . z-max))
         (clip-line-z #t z-max x1 y1 z1 x2 y2 z2)]
        [(and (z2 . <= . z-max) (z1 . > . z-max))
         (clip-line-z #f z-max x1 y1 z1 x2 y2 z2)]
        [else
         (values x1 y1 z1 x2 y2 z2)]))
...
(define (clip-line v1 v2 x-min x-max y-min y-max z-min z-max)
  (let/ec return
    ...
    (when (or (and (x1 . < . x-min) (x2 . < . x-min))
              (and (x1 . > . x-max) (x2 . > . x-max))
              (and (y1 . < . y-min) (y2 . < . y-min))
              (and (y1 . > . y-max) (y2 . > . y-max))
              (and (z1 . < . z-min) (z2 . < . z-min))
              (and (z1 . > . z-max) (z2 . > . z-max)))
      (return #f #f))
    (let*-values
        ([(x1 y1 z1 x2 y2 z2)  (clip-line-x-min x-min x1 y1 z1 x2 y2 z2)]
     ...
     (cond [(and v1-in-bounds? v2-in-bounds?)  (cons v2 res)]
           [(and (not v1-in-bounds?) (not v2-in-bounds?))  res]
            [else
             (match-define (vector x1 y1 z1) v1)
             (match-define (vector x2 y2 z2) v2)
             (let-values ([(x1 y1 z1 x2 y2 z2)
                           (clip-line v1-in-bounds? val x1 y1 z1 x2 y2 z2)])
               (if v2-in-bounds?
                   (list* (vector x2 y2 z2) (vector x1 y1 z1) res)
                   (cons (vector x2 y2 z2) res)))]))))
...
(define (clip-polygon vs x-min x-max y-min y-max z-min z-max)
  (let/ec return
    ...
    (when (or (andmap (λ (x) (x . < . x-min)) xs)
              ...
              (andmap (λ (y) (y . < . y-min)) ys)
              (andmap (λ (y) (y . > . y-max)) ys)
              (andmap (λ (z) (z . < . z-min)) zs)
              (andmap (λ (z) (z . > . z-max)) zs))
      (return empty))
