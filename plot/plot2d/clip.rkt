...
(define (clip-line v1 v2 x-min x-max y-min y-max)
  (let/ec return
    ...
    (when (or (and (x1 . < . x-min) (x2 . < . x-min))
      ...
      (return #f #f))
    (let*-values
        ([(x1 y1 x2 y2)  (clip-line-x-min x-min x1 y1 x2 y2)]
