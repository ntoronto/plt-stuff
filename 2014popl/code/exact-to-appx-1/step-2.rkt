...
;; Changes from step-1.rkt:
;;  * Removed function arrow from tests (no need for it - mapping arrow preimages tested correct)
;;  * Moved applying `preimage-close' to just before running a computation

;; Proof obligations:
;;  * Moving `preimage-close' outward doesn't change computed preimages
...
(define (test-comps map-comp pre-comp A B print?)
  (for: ([A  (in-list (set->list (set-power A)))])
    (define pre (preimage-close (pre-comp A)))
