...
(define (mapping-ap f x)
  (cdr (set-take (set-filter (λ: ([xy : (Pair X Y)]) (equal? x (car xy))) f))))
...
(define (mapping-compose f g)
  (mapping (λ: ([x : X]) (mapping-ap f (mapping-ap g x)))
           (mapping-domain g)))
...
(struct: (X Y) lazy-mapping ([function : (X -> Y)]
                             [domain : (Setof X)])
  #:transparent)
...
(define (mapping->lazy f)
  (lazy-mapping (λ: ([x : X]) (mapping-ap f x))
                (mapping-domain f)))
...
(define (lazy-mapping-ap f x)
  ...
  (cond [(set-member? A x)  (g x)]
        [else  (raise-argument-error 'lazy-mapping-ap "domain value" x)]))
