...
(: mapping-ap (All (X Y) ((Mapping X Y) X -> Y #;(U Y Bottom))))
(define (mapping-ap f x)
  (define A (set-filter (λ: ([xy : (Pair X Y)]) (equal? x (car xy))) f))
  (cond [(set-empty? A)  (error 'bad) #;bottom]
        [else  (cdr (set-take A))]))

(: mapping-ap/error (All (X Y) ((Mapping X Y) X -> Y)))
(define (mapping-ap/error f x)
  (define y (mapping-ap f x))
  (cond [(bottom? y)  (raise-argument-error 'mapping-ap/error "in-domain value" 1 f x)]
        [else  y]))

(: mapping-image (All (X Y) ((Mapping X Y) (Setof X) -> (Setof Y))))
(define (mapping-image f A)
  (set-image (λ: ([x : X]) (mapping-ap/error f x))
...
(define (mapping-compose f g)
  (define h (set-image (λ: ([xy : (Pair X Y)])
                         (define y (cdr xy))
                         (define z ((inst mapping-ap Y Z) f y))
                         (if (bottom? z) bottom (cons (car xy) z)))
                       g))
  (set-filter-out bottom? h))
