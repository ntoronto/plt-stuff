...
\end{theorem}
\begin{proof}
The bottom arrow is isomorphic to the Maybe monad's Kleisli arrow with $Nothing = \bot$.
\end{proof}
...
\begin{corollary}[semantic$\conv$ correctness]
...
\end{theorem}
\begin{proof}
By preimage/projection monotonicity and Lemma~\ref{lem:preimage-under-pairing}.
\end{proof}
...
Suppose we defined ${\convifppre}'~k_1~k_2~k_3~j~A$ to take \emph{no branches} when $branch\ppre~j~A$ contains both $true$ and $false$.
...
\\[-6pt]
	&\begin{aligned}[t]
		fst\ppre' := \arrowtrans\ppre'~fst\pre;\ \ snd\ppre' := \arrowtrans\ppre'~snd\pre;\ \ \cdots
	\end{aligned}
\end{aligned}
&\tab\tab
\begin{aligned}[t]
	&\begin{aligned}[t]
		...
		}{if~(C_b = \set{true,false})~\pair{\top,\fun{B}\lzfcsplit{A_2 \join A_3}}~(k_2~j_2~A_2 \uplus\pre' k_3~j_3~A_3)}
	\end{aligned}
\end{aligned}
\end{align*}
\hrule
...
Figure~\ref{fig:approximating-preimage-arrow-defs} shows the final approximating preimage arrow.
...
For $ap\pre'~(h'~j_0~A)~B$, lattice operations, which replace set operations in the approximating combinators, are monotone.\end{proof}
...
\subsection{Approximation Limitations}

With disjointness and monotonicity properties, it is natural to suppose that we can compute probabilities of preimages of $B$ by computing preimages with respect to increasingly fine discretizations.
...
\end{enumerate}
If the second step always yields finer partitions, it seems the sum of the measures of each rectangle in $\A$ should approach the probability of $B$.

We are not so fortunate: in the limit, we would be computing the \keyword{Jordan outer measure} of the preimage of $B$, which is not always equal to its probability.
...
The preimage of $\set{true}$ under $\meaningof{rational?~random}\pmap\conv$ has probability $0$, but its outer Jordan measure is $1$.

Roughly, it is impossible to tightly cover a dense, measurable set with finite partitions.

Fortunately, finite discretization tends to work out anyway.
In a language with reasonable primitives, programs that halt with probability 1 tend to be piecewise continuous, so preimages of rectangles are finite unions of Jordan-measurable sets.
These unions are thus Jordan-measurable, so their outer Jordan measure is their probability.

Still, there is one very good reason to not exhaustively discretize.
Good accuracy would require fine discretization, which is \emph{exponential} in the number of discretized axes.
For example, a nonrecursive program that contains only 10 uses of $random$ would need to partition 10 axes of $R$.
Splitting each into only 4 intervals yields a partition of $R$ of size $4^{10} = 1,048,576$.

This explosion in time and space is often called the \keyword{curse of dimensionality}.
One way to avoid it is to sample instead of enumerate, which we do in our main implementation.
...
Given a rectangular set library, the approximating semantics defined in Figure~\ref{fig:extra-preimage-arrow-defs} and Figure~\ref{fig:approximating-preimage-arrow-defs} can be implemented with few changes in almost any practical $\lambda$-calculus.
We have done so in both Typed Racket~[XXX:cite] and Haskell~[XXX:cite].

In Typed Racket, we have also implemented $\meaningof{\cdot}\gen$ from Figure~\ref{fig:semantic-function} and its extension $\meaningof{\cdot}\gen\conv$, targeting both the bottom* arrow and the preimage* arrow.
