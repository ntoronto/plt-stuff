...
\\
	\meaningof{env~(\mathit{n}+1)}\gen &\ :\equiv\
		(\arrowarr\gen~snd)~\arrowcomp\gen~\meaningof{env~\mathit{n}}\gen
\\
\end{aligned}
&\tab\tab\tab
\begin{aligned}[t]
...
\end{align*}
\hrule
\caption{Transformation from a let-calculus with De-Bruijn-indexed bindings to arrow computations, for any arrow $\mathit{a}$.
