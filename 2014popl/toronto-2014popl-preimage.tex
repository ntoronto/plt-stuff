...
\newcommand{\pmap}{_\mathrm{map^*}}
\DeclareMathOperator{\pmapto}{\arrow_{\mspace{-21mu}\pmap}}
...
@V{\eta_\bot}VV             @VV{\eta\map}V              @VV{\eta\pre}V\\
X \botto' Y  @>>\liftmap'>  X \mapto' Y  @>>\liftpre'>  X \preto' Y
\end{CD}
\end{equation}
...
Formal goals:
\begin{enumerate}
	\item Preimages of pure, total functions computed using $X \preto' Y$ are the preimages under $X \botto Y$ (have this, by homomorphism)
	\item Preimages computed using $X \preto' Y$ are the preimages under $X \botto' Y$ (need to prove $\liftmap'$ and $\liftpre'$ are homomorphisms)
	\item Running $X \botto' Y$ and $X \preto' Y$ computations always converges
\end{enumerate}
...
$\arrowtrans\genc$ is an arrow+choice homomorphism.
\end{theorem}
\begin{proof}
...
We still need to know that preimages under functions that use the store are computed correctly, which we will get to after defining stores and combinators that access them.


\subsection{Probabilistic Programs}

Define $R := J \to [0,1]$ as the set of infinite random sources.
Each $r \in R$ is a total mapping from an expression index to a random number.
Equivalently, each $r \in R$ is an infinite vector of random numbers indexed by $J$.
...
\end{equation}
for arrows $b$ for which $random\genc$ is defined.
...
Roughly, the store determines which branch of each conditional, if any, is allowed to continue computing.

\begin{definition}[branch trace]
A \mykeyword{branch trace} is a total mapping $t \in J \to Bool_\bot$ such that $t~j = true$ or $t~j = false$ for no more than finitely many $j \in J$.
\end{definition}

Let $T$ be the largest subset of branch traces in $J \to Bool_\bot$, and $x \arrow\genc y ::= AStore~T~(x \arrow\gen y)$.
...
\end{aligned}
\end{equation}
Using $branch\genc$, we define an additional if-then-else combinator, which checks to see whether its conditional expression agrees with the branch trace:
\begin{align}
	&\begin{aligned}
		&agrees : \pair{Bool,Bool} \tto Bool_\bot \\
		&agrees~\pair{a_1,a_2} \ := \ if~(a_1 = a_2)~a_1~\bot
	\end{aligned} \\
\nonumber \\[-6pt]
	&\begin{aligned}
		&\arrowif\genc' : (x \arrow\genc Bool) \tto (x \arrow\genc y) \tto (x \arrow\genc y) \tto (x \arrow\genc y) \\
...
We will prove that this correctly returns $\bot$ instead of diverging after defining the meaning of partial, probabilistic let-calculus programs.

\subsection{Partial, Probabilistic Programs}

Let $S ::= R \times T$ and $x \arrow\genc y ::= AStore~S~(x \arrow\gen y)$.
Update the $random\genc$ and $branch\genc$ combinators to reflect that the store is now a pair:
\begin{align}
	&\begin{aligned}
		&random\genc : x \arrow\genc [0,1] \\
...
We define a new semantic function $\meaningof{\cdot}\genc'$ by extending $\meaningof{\cdot}\genc$:
\begin{equation}
\begin{aligned}
...
Let $\arrowlift\genb : (x \arrow\gen y) \tto (x \arrow\genb y)$ be an arrow+choice homomorphism, and define
\begin{equation}
\begin{aligned}
...
\end{CD}
\end{equation}
or for all $f : x \arrow\gen y$, $\arrowtrans\gend~(\arrowlift\genb~f) \equiv \arrowlift\gend~(\arrowtrans\genc~f)$.
Further, $\arrowlift\gend$ is an arrow+choice homomorphism.
\end{theorem}
\begin{proof}
...
\end{align*}
Lastly, because $\arrowtrans\genc$, $\arrowtrans\gend$, and $\arrowlift\genb$ are homomorphisms, $\arrowlift\gend$ is a homomorphism by composition.
...
\begin{equation}
\begin{CD}
X \botto Y   @>\liftpre \circ \liftmap>> X \preto Y \\
@V{\eta_\pbot}VV            @VV{\eta\ppre}V\\
X \pbotto Y  @>>\liftppre \circ \liftpmap>  X \ppreto Y
\end{CD}
\end{equation}
Further, $\liftppre \circ \liftpmap$ is an arrow+choice homomorphism.
\end{corollary}

In particular, $\meaningof{\mathit{e}}\ppre'$ correctly computes preimages under the interpretation of $\mathit{e}$ as a partial function from an infinite random source.
