...
The homomorphism laws state that $\arrowarr\gen$ must be a homomorphism from the function arrow~\eqref{eqn:function-arrow} to arrow $a$.
...
Let $h : X \preto Y$. If there exists an $g : X \mapto Y$ such that $h \equiv \liftpre~g$, then $h$ obeys the \mykeyword{preimage arrow law}.
...
We thread a random store through bottom, mapping, and preimage arrow computations by defining an \keyword{arrow transformer}: a type constructor that receives and produces an arrow type, and combinators for arrows of the produced type. (XXX: find better cite than the experimental-looking Hackage package)
...
\end{equation}
Reading the type, we see that computations receive an index and produce a computation that receives a store as well as an $x$.
...
\end{itemize}
The maximal domain of $k : X \arrow\genc Y$ is that of $k~j_0$, and is a subset of $(R \times T) \times X$.
...
\begin{theorem}[divergence as an error]
...
\begin{definition}[maximal probability]
...
The \keyword{maximal probability} of $g$ is $P~(image~(fst~\arrowcomp~fst)~A^*)$.
\end{definition}
...
We are not so fortunate: in the limit, we would be computing the \keyword{Jordan outer measure}~(XXX: cite) of the preimage of $B$, which is not always equal to its measure.
