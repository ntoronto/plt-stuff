...
XXX: One of the strengths of measure-theoretic probability is that it can be used to factor the definition of a random process into an entirely deterministic part and an assumed-random part
...
\begin{definition}[topology, open set]
A collection of sets $\A \subseteq \powerset~X$ is called a \keyword{topology} on $X$ if it contains $\emptyset$ and $X$, and is closed under finite intersections and arbitrary unions.
The sets in $\A$ are called \keyword{open sets}.
\end{definition}
...
\begin{definition}[continuous/measurable mapping]
Let $\A$ and $\B$ be topologies/$\sigma$-algebras on $X$ and $Y$ respectively.
A mapping $g : X \pto Y$ is $\A!\B$-\keyword{continuous/measurable} if for all $B \in \B$, $preimage~g~B \in \A$.
\end{definition}

%Note that $domain~g = preimage~g~Y$, so $domain~g \in \A$.
...
Topologies and $\sigma$-algebras on product spaces are defined the same way.

\begin{definition}[finite product topology/$\sigma$-algebra]
Let $\A_1$ and $\A_2$ be topologies/$\sigma$-algebras on $X_1$ and $X_2$, and $X := \pair{X_1,X_2}$.
The \keyword{product topology/$\sigma$-algebra} $\A_1 \otimes \A_2$ is the smallest topology/$\sigma$-algebra for which $fst\restrict{X}$ and $snd\restrict{X}$ are continuous/measurable mappings.
\end{definition}

\begin{definition}[arbitrary product topology/$\sigma$-algebra]
Let $\A$ be a topology/$\sigma$-algebra on $X$.
The \keyword{product topology/ $\sigma$-algebra} $\A^{\otimes J}$ is the smallest topology/$\sigma$-algebra for which, for all $j \in J$, $(\pi~j)\restrict{J \to X}$ is a continuous/measurable mapping.
\end{definition}

\begin{definition}[measurable computation]
Let $\A$ and $\B$ be $\sigma$-algebras for $X$ and $Y$ respectively.
A mapping arrow computation $g : X \mapto Y$ is $\A!\B$-\keyword{measurable} if for all $A \subseteq A^*$, $g~A$ is an $\A!\B$-measurable mapping, where $A^*$ is $g$'s halting set.
\end{definition}

Notice that the $A$ in $g~A$ need not be measurable, but $domain~(g~A)$ must be.
XXX: this implies $\arrmap~fst$ is not measurable!
...
\end{theorem}
\begin{proof}
Recall the definition of $(\compmap)$:
\begin{equation}
	(g_1~\compmap~g_2)~A \ := \ 
		\lzfclet{
			g_1' & g_1~A \\
...
\end{equation}
By definition, $g_1~A$ is an $\A!\B$-measurable mapping; likewise for $g_2~(range~g_1')$.
Apply Lemma~\ref{lem:compositions-are-measurable}.
\end{proof}
...
\end{theorem}
\begin{proof}
Recall the definition of $(\pairmap)$:
\begin{equation}
	(g_1~\pairmap~g_2)~A \ := \ \pair{g_1~A,g_2~A}\map
\end{equation}
By definition, $g_1~A$ is an $\A!\B_1$-measurable mapping; likewise for $g_2~A$.
Apply Lemma~\ref{lem:pairings-are-measurable}.
\end{proof}
...
\end{theorem}
\begin{proof}
Recall the definition of $\ifmap$:
\begin{equation}
	\ifmap~g_1~g_2~g_3~A \ := \ 
		\lzfclet{
			g_1' & g_1~A \\
...
\end{equation}
Clearly $g_1'$, $g_2'$ and $g_3'$ are measurable mappings, and $g_2'$ and $g_3'$ have disjoint domains.
Apply Lemma~\ref{lem:disjoint-unions-are-measurable}.

XXX: this assumes $domain~g_2' \subseteq preimage~g_1'~\set{true}$, similarly for $g_3'$
\end{proof}
...
\end{theorem}
\begin{proof}
Recall the definition of $\lazymap$:
\begin{equation}
	\lazymap~g~A \ := \ if~(A = \emptyset)~\emptyset~(g~0~A)
\end{equation}
Let $A \subseteq A^*$.
...
\subsection{Every Program Is Measurable}

\begin{definition}[standard $\sigma$-algebra]
For $X$ used in a type, $\Sigma~X$ denotes its \mykeyword{standard $\sigma$-algebra}, defined by
\begin{align}
...
XXX: assume every lifted function is measurable
...
If $A^*$ is measurable, the \keyword{halting probability} of $f$ is $P~(image~(fst~\arrowcomp~fst)~A^*)$.
\end{definition}
