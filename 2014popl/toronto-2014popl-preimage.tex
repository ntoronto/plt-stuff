...
The absence of $\bot$ and the fact that the type parameters are sets will make it easier to apply theorems from set theory, which know nothing of error values and lambda types.
...
XXX: remind readers that lemms are imported theorems
...
\\[-6pt]
	&\begin{aligned}[t]
		%&agrees\pre : \pair{Bool,Bool} \preto Bool \\
...
\begin{definition}[rectangular subset]
For a set $X$ used as a type, $Rect~X$ denotes the \mykeyword{rectangular subsets} of $X$, which must be satisfy the following rules:
\begin{align}
