...
\newcommand{\lazybot}{\arrowlazy_\bot}
...
One way to be certain an arrow correctly computes preimages under functions $f$ is to ultimately \emph{derive} it from a simpler arrow used to construct $f$.
One obvious candidate is the \keyword{function arrow}, in which $x \arrow y ::= x \tto y$, whose implementation is extremely simple (e.g. $f_1~\arrowcomp~f_2$ is $f_2 \circ f_1$).
...
Its computations are of type $x \tto y_\bot$, where the inhabitants of $y_\bot$ are the error value $\bot$ as well as the inhabitants of $y$. The type $Bool_\bot$, for example, denotes the members of $Bool \u \set{\bot}$.
...
\begin{aligned}[t]
	&\begin{aligned}[t]
		&\ifbot : (x \tto Bool_\bot) \tto (x \tto y_\bot) \tto (x \tto y_\bot) \tto (x \tto y_\bot) \\
		&\ifbot~f_1~f_2~f_3~x \ := \
			\lzfccase{f_1~x}{true & f_2~x \\ false & f_3~x \\ else & \bot}
	\end{aligned} \\
\\[-6pt]
	&\begin{aligned}[t]
		&\lazybot : (1 \tto (x \tto y_\bot)) \tto (x \tto y_\bot) \\
		&\lazybot~f~x \ := \ f~0~x
	\end{aligned}
\end{aligned}
\end{align*}
...
Figure~\ref{fig:bottom-arrow-defs} does not give the typical minimal definition consisting of $\arrowarr$, $(\arrowcomp)$ and $first$ combinators.
Instead of $first_\bot$, it defines $(\arrowpair_\bot)$---typically called \keyword{fanout}, but its use in this paper will be clearer if we call it \mykeyword{pairing}---which applies two functions to the same input and returns the pair of their outputs.
Though any arrow's $first$ may be defined in terms of its $(\arrowpair)$ and vice-versa~\cite{cit:hughes-2005afp-arrows}, we give $(\arrowpair)$ definitions in this paper because the most well-known applicable contemporary measurability theorems are in terms of pairing functions.

Figure~\ref{fig:bottom-arrow-defs} also defines the \mykeyword{bottom arrow+choice} by defining the additional combinators $\ifbot$ (if-then-else) and $\lazybot$.
An arrow is typically strengthened to an arrow+choice (which is not quite as strong as a monad) by defining a $left$ combinator.
Again, however, defining $\ifbot$ instead of $left_\bot$ will make it easier to apply contemporary measurability theorems, which are in terms of disjoint unions of mappings instead of an explicit disjoint union type.

In a nonstrict or simply typed $\lambda$-calculus, $\lazybot$ is unnecessary.
For example, in a simply typed $\lambda$-calculus, the following recursive function cannot be typed:
\begin{equation}
	halt@on@true_\bot \ := \ \ifbot~(\arrbot~id)~(\arrbot~id)~halt@on@true_\bot
\end{equation}
and it diverges in a nonstrict $\lambda$-calculus only when applied to $false$.
However, its \emph{defining expression} diverges in \lzfclang and every other call-by-value $\lambda$-calculus.

We defer the inner $halt@on@true_\bot$ until after the outer $halt@on@true_\bot$ is applied, using $\lazybot$:
\begin{equation}
\begin{aligned}
	&halt@on@true_\bot \ := \ \\
	&\tab\ifbot~(\arrbot~id)~(\arrbot~id)~(\lazybot~\fun{0}{halt@on@true_\bot})
\end{aligned}
\end{equation}
This diverges only when applied to $false$ in any sensible $\lambda$-calculus.

XXX: point out that $\lazybot$ receives a thunk, and remind readers that $1 = \set{0}$
...
\end{theorem}
\begin{proof}
The bottom arrow is the arrow in the Kleisli category of the Maybe monad with $Nothing = \bot$.
\end{proof}

\section{First-Order Let-Calculus Semantics}
...
It is tempting to try to define the mapping arrow's type constructor using $X \mapto Y ::= X \pto Y$, and define $(\pairmap) := \pair{\cdot,\cdot}\map$ and $(\compmap) := flip~(\circ\map)$.
Unfortunately, we run into a problem defining $\arrmap : (X \tto Y) \tto (X \pto Y)$: we cannot define it as $\arrmap~f := f\restrict{X}$. Although $X$ is a \lzfclang value, it is not available within any definition because it is only part of the type.

We need to parameterize computations on a domain, so
\begin{equation}
	X \mapto Y \ ::= \ Set~X \tto (X \pto Y)
\end{equation}
is the type of \mykeyword{mapping arrow} computations.
...
We want the correspondence between bottom arrow and mapping arrow computations as clear as possible.
We therefore start by defining a function $\liftmap : (X \tto Y_\bot) \tto (X \mapto Y)$ to lift bottom arrow computations to the mapping arrow.
...
More concretely, for any let-calculus expression $\mathit{e}$, we would like $\meaningof{\mathit{e}}\map \equiv \liftmap~\meaningof{\mathit{e}}_\bot$.
...
\end{definition}
Recall that in nonstrict or strongly normalizing languages, a $\arrowlazy$ combinator is unnecessary.
In these languages, only~\eqref{eqn:lift-distributes-over-if} is necessary for arrow+choice homomorphisms.
...
With $\ifmap$ and $\lazymap$, they define an arrow+choice.
\end{corollary}
...
\end{theorem}
\begin{proof}
Apply Lemma~\ref{lem:preimage-restricted-range} with $Y' = range~g$.
\end{proof}
...
The following are easy consequences of the fact that $\liftpre$ is a homomorphism.

\begin{corollary}
$\arrpre$, $(\pairpre)$ and $(\comppre)$ define an arrow. With $\ifpre$ and $\lazypre$, they define an arrow+choice.
\end{corollary}
...
\begin{corollary}
If $\meaningof{\mathit{e}}\map : X \mapto Y$, then for all $A \subseteq X$ and $B \subseteq Y$, $preimage~(\meaningof{\mathit{e}}\map~A)~B \equiv pre@ap~(\meaningof{\mathit{e}}\pre~A)~B$.
\end{corollary}

In other words, $\meaningof{\mathit{e}}\pre$ returns a computation that correctly computes preimages under $\meaningof{\mathit{e}}\map$ or, again by homomorphism, $\meaningof{\mathit{e}}_\bot$.
...
They often come up when practitioners want to build data with random size or structure, but they may show up in simpler circumstances as well.
...
Divergence simply does not happen in practice.

Suppose we interpret~\eqref{eqn:geometric-def} as $h : X \preto \Nat$, a preimage arrow computation from random sources in $X$ to natural numbers.
Given a uniform probability measure $P \in \powerset~X \pto [0,1]$, we could compute the probability of some output set $N \subseteq \Nat$ using $P~(h~A~N)$, where $A \subseteq X$, $P~A = 1$ and $h~A$ converges.

We have three hurdles to overcome:
\begin{enumerate}
	\item Determining how $boolean~p$ retrieves unique real numbers $x_i$ from $x$.
	\item Ensuring that each $x \in X$ contains enough random numbers.
	\item Ensuring that $h~A$ converges.
\end{enumerate}
For hurdle 3, we will ensure that $h~A$ converges for any $A \subseteq X$.
Hurdle 2 is easy: we make each $x$ infinite.
Hurdle 1 relies on an expression indexing scheme for unrolled programs, which we develop to address 3.
