...
In each case, the resulting definitions are large, conceptually difficult, and hard to formally manipulate.
Fortunately, assigning each sub-computation a unique index into a tree-shaped, \emph{constant} random source, and passing the source applicatively, is relatively easy.
...
If for every finite composition $f$ of $left$ and $right$, $f~j_0$ is unique, then $J$, $j_0$, $left$ and $right$ define a \mykeyword{binary indexing scheme}.
\end{definition}
...
XXX: computations receive an index and return an arrow from a store paired with $x$ to $y$:
\begin{equation}
	AStore~(x \arrow\gen y) \ ::= \ J \tto \pair{s,x} \arrow\gen y
\end{equation}

XXX: motivational wurds for definition:
\begin{equation}
\begin{aligned}
	&\arrowlift\genb : x \arrow\gen y \tto AStore~(x \arrow\gen y) \\
	&\arrowlift\genb~f~j \ := \ \arrowarr\gen~snd~\arrowcomp\gen~f
\end{aligned}
\end{equation}
...
\begin{aligned}[t]
	&\begin{aligned}[t]
		x \arrow\genb y \ ::= \ AStore~(x \arrow\gen y) \ ::= \ J \tto (\pair{s,x} \arrow\gen y)
	\end{aligned} \\
\\[-6pt]
	&\begin{aligned}[t]
		&\arrowarr\genb : (x \tto y) \tto (x \arrow\genb y) \\
		&\arrowarr\genb \ := \ \arrowlift\genb \circ \arrowarr\gen
	\end{aligned} \\
\\[-6pt]
	&\begin{aligned}[t]
		&(\arrowcomp\genb) : (x \arrow\genb y) \tto (y \arrow\genb z) \tto (x \arrow\genb z) \\
		&(k_1~\arrowcomp\genb~k_2)~j \ := \
			(\arrowarr\gen~fst~\arrowpair\gen~k_1~(left~j))~\arrowcomp\gen~k_2~(right~j)
	\end{aligned} \\
\\[-6pt]
	&\begin{aligned}[t]
		&(\arrowpair\genb) : (x \arrow\genb y) \tto (x \arrow\genb z) \tto (x \arrow\genb \pair{y,z}) \\
		&(k_1~\arrowpair\genb~k_2)~j \ := \ k_1~(left~j)~\arrowpair\gen~k_2~(right~j)
	\end{aligned} \\
\end{aligned}
&\tab\tab\tab
\begin{aligned}[t]
	&\begin{aligned}[t]
		&(\arrowif\genb) : (x \arrow\genb Bool) \tto (x \arrow\genb y) \tto (x \arrow\genb y) \tto (x \arrow\genb y) \\
		&\arrowif\genb~k_1~k_2~k_3~j \ := \
			\lzfcsplit{\arrowif\gen~&(k_1~(left~j)) \\ &(k_2~(left~(right~j))) \\ &(k_3~(right~(right~j)))}
	\end{aligned} \\
\\[-6pt]
	&\begin{aligned}[t]
		&\arrowlazy\genb : (1 \tto (x \arrow\genb y)) \tto (x \arrow\genb y) \\
		&\arrowlazy\genb~k~j \ := \ \arrowlazy\gen~\fun{0}{k~0~j}
	\end{aligned} \\
\\[-6pt]
\hline
\\[-6pt]
	&\begin{aligned}[t]
		&\arrowlift\genb : (x \arrow\gen y) \tto (x \arrow\genb y) \\
		&\arrowlift\genb~f~j \ := \ \arrowarr\gen~snd~\arrowcomp\gen~f
	\end{aligned}
\end{aligned}
\end{align*}
...
Again, to do so, we need to extend equivalence to be more extensional for arrows $AStore~(x \arrow\gen y)$.
...
Let $x \arrow\genb y ::= AStore~(x \arrow\gen y)$.
$\arrowlift\genb$ is an arrow+choice homomorphism.
\end{theorem}
\begin{proof}
Defining $\arrowarr\genb$ as a composition clearly meets the first homomorphism identity~\eqref{eqn:lift-distributes-over-arr}.
...
\begin{align*}
	&(\arrowlift\genb~f_1~\arrowcomp\genb~\arrowlift\genb~f_2)~j
\\
	&\tab\equiv\ 
...
\\
	&\tab\equiv\
		\arrowlift\genb~(f_1~\arrowcomp\gen~f_2)~j
\end{align*}
...
\begin{align*}
	&(\arrowlift\genb~f_1~\arrowpair\genb~\arrowlift\genb~f_2)~j
\\
	&\tab\equiv\ 
...
\\
	&\tab\equiv\
		\arrowlift\genb~(f_1~\arrowpair\gen~f_2)~j
\end{align*}
...
\begin{align*}
	&(\arrowif\genb~(\arrowlift\genb~f_1)~(\arrowlift\genb~f_2)~(\arrowlift\genb~f_3))~j
\\
	&\tab\equiv\ 
...
\\
	&\tab\equiv\ 
		\arrowlift\genb~(\arrowif~f_1~f_2~f_3)~j
\end{align*}
...
\begin{align*}
	&(\arrowlazy\genb~\fun{0}{\arrowlift\genb~(f~0)})~j
\\
	&\tab\equiv\ 
...
\\
	&\tab\equiv\ 
		\arrowlift\genb~(\arrowlazy\gen~f)~j
\end{align*}
\end{proof}

\begin{corollary}[semantic correctness]
Let $x \arrow\genb y ::= AStore~(x \arrow\gen y)$. If $\meaningof{\mathit{e}}\gen : x \arrow\gen y$, then $\arrowlift\genb~\meaningof{\mathit{e}}\gen \equiv \meaningof{\mathit{e}}\genb$ and $\meaningof{\mathit{e}}\genb : x \arrow\genb y$.
\end{corollary}

\subsection{Probabilistic Programs}

Define $\Omega := J \to [0,1]$ as the set of infinite random sources
...
\begin{equation}
\begin{aligned}
	&random\genb : x \arrow\genb [0,1] \\
	&random\genb~j \ := \ \arrowarr\gen~fst~\arrowcomp\gen~\arrowarr\gen~(\pi~j)
\end{aligned}
\end{equation}

\begin{equation}
	\meaningof{random}\genb \ :\equiv \ random\genb
\end{equation}
...
Let $Z \subset J \to Bool$ be the set of $z \in J \to Bool$ such that $z~j = true$ for only finitely many $j$

Store type: $s ::= Z$

\newcommand{\arrowgetlazy}{\ensuremath{getz}}

\begin{equation}
\begin{aligned}
	&\arrowgetlazy\genb : x \arrow\genb Bool \\
	&\arrowgetlazy\genb~j \ := \ \arrowarr\gen~fst~\arrowcomp\gen~\arrowarr\gen~(\pi~j)
\end{aligned}
\end{equation}
...
\begin{aligned}
	&\arrowlazy\genb' : (1 \tto (x \arrow\genb y)) \tto (x \arrow\genb y) \\
	&\arrowlazy\genb'~k \ := \ \arrowlazy\genb~\fun{0}{\arrowif\genb~\arrowgetlazy\genb~(k~0)~(\arrowarr\genb~\fun{x}{\bot})}
\end{aligned}
\end{equation}

\begin{equation}
		\meaningof{if~\mathit{e_c}~\mathit{e_t}~\mathit{e_f}}\genb' \ :\equiv\
		\arrowif\genb~
			\meaningof{\mathit{e_c}}\genb~
			(\arrowlazy\gen'~\fun{0}{\meaningof{\mathit{e_t}}\genb})~
			(\arrowlazy\gen'~\fun{0}{\meaningof{\mathit{e_f}}\genb})
\end{equation}


\subsection{Partial, Probabilistic Programs}

Store type: $s ::= \pair{Z, \Omega}$

\begin{equation}
\begin{aligned}
	&\arrowgetlazy\genb : x \arrow\genb Bool \\
	&\arrowgetlazy\genb~j \ := \ \arrowarr\gen~fst~\arrowcomp\gen~\arrowarr\gen~fst~\arrowcomp\gen~\arrowarr\gen~\pi~j
\end{aligned}
\end{equation}
...
\begin{aligned}
	&random\genb : x \arrow\genb [0,1] \\
	&random\genb~j \ := \ \arrowarr\gen~fst~\arrowcomp\gen~\arrowarr\gen~snd~\arrowcomp\gen~\arrowarr\gen~(\pi~j)
\end{aligned}
\end{equation}
