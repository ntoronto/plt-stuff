...
Baking such support into $\meaningof{\cdot}\gen$ might complicate the simple proof of the following theorem, which underlies most of our semantic correctness claims. (XXX: just say it's trivial and well-understood)
...
\end{proof}
XXX: restructure to have the proofs go left-to-right
...
To use Theorem~\ref{thm:homomorphism-implies-correct} to prove correct the interpretations of programs as preimage arrow computations, we need the preimage arrow to be homomorphic to a simpler arrow whose behavior is obviously correct.
...
We define $\meaningofconv{\cdot}\genc$ as nearly identical to $\meaningof{\cdot}\genc$, but with a different $if$ rule:
\begin{equation}
\begin{aligned}
...
Computing preimages and ``projecting out'' $T$ and $X$ should preserve measurability.
...
(XXX: ``If you know topology, we relate it that, if you don't need this, skip to xxx'')
...
\end{equation}
where $A \in \Sigma~\pair{\pair{R,T},X}$ and $domain~P = \Sigma~R$. 
Unfortunately, projected sets are generally not measurable.
Fortunately, if $X$ is at most countable and $\Sigma~X = \powerset~X$, we have a special case.
