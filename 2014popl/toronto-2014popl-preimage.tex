...
Figure~\ref{fig:bottom-arrow-defs} also defines the \mykeyword{bottom arrow+choice} by defining the additional combinators $\ifbot$ and $\lazybot$.
Typically, an arrow is strengthened to an arrow+choice---which is not quite as strong as a monad---by defining a $left$ combinator.
Again, however, our choice of $\ifbot$ will make it easier to apply contemporary measurability theorems, which are in terms of disjoint unions of mappings instead of an explicit disjoint union type.
...
\\
	\meaningof{if~\mathit{e_c}~\mathit{e_t}~\mathit{e_f}}\gen &\ :\equiv\
		if\gen~
			...
			(\arrowlazy~\fun{0}{\meaningof{\mathit{e_t}}\gen})~
			(\arrowlazy~\fun{0}{\meaningof{\mathit{e_f}}\gen})
\\
\end{aligned}
&\tab\tab\tab
\begin{aligned}[t]
...
It is helpful to have a standalone function $domain_\bot$ that computes such domains, so we define that first:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
and define $\liftmap$ in terms of $domain_\bot$:
\begin{equation}
\begin{aligned}
...
In other words, $\meaningof{\mathit{e}}\pre$ returns a computation that correctly computes preimages under $\meaningof{\mathit{e}}\map$ or, again by homomorphism, $\meaningof{\mathit{e}}_\bot$ (without error inputs).
