...
Measure-theoretic probability~\cite{cit:klenke-2006-probability} is widely believed to be able to define everything reasonable that densities cannot.
...
For difficulty~\ref{problem:higher-orderness}, we have discovered that the ``first-orderness'' of arrows~\cite{cit:hughes-2005afp-arrows} is a perfect fit for the ``first-orderness'' of measure theory.
...
The computations of the arrows in the bottom row are like those in the top, except they thread an infinite store of random values, and always converge.
...
All operations, including operations on infinite sets, are assumed to complete instantly if they converge.
...
Further, proofs about \lzfclang's set values apply directly to ZFC sets, assuming the existence of a single inaccessible cardinal.\footnote{A mild assumption, as ZFC+$\kappa$ is a smaller theory than Coq's~\cite{cit:barras-2010-sets-coq}, and algebraic geometers assume a proper class of inaccessibles.}

In \lzfclang, algebraic data structures are encoded as sets; e.g. $\pair{x,y}$ can be encoded as $\set{\set{x},\set{x,y}}$.
...
In these cases, we use something slightly weaker.
...
We call extensional functions \keyword{mappings} and intensional functions \keyword{lambdas}, and use the word \keyword{function} to mean either.
...
Like monads and idioms~\cite{cit:wadler-2001-monads,cit:mcbride-2008jfp-idiom}, arrows~\cite{cit:hughes-2005afp-arrows} are used in functional programming to thread effects through computations in a way that imposes structure on the computations.
...
Equivalence between different arrow representations is usually proved in a strongly normalizing $\lambda$-calculus~\cite{cit:lindley-2008entcs-idiom-arrow-monad,cit:lindley-2010jfp-arrow-calculus}, in which every function is free of effects, including divergence.
...
The pair extraction law~\eqref{eqn:pair-extraction}, which \emph{can} be derived from existing arrow laws, is a little more problematic, in nonstrict $\lambda$-calculii as well as \lzfclang.
If $f_1$ can diverge, using~\eqref{eqn:pair-extraction} to transform a computation can turn a diverging expression into a converging one, or vice-versa.
We could condition the pair extraction law on $f_1$'s totality.
Instead, we require every argument to $\arrowarr\gen$ to be total, which simplifies more proofs.
...
If $f : X \botto Y$, $g : X \mapto Y$ and $h : X \preto Y$, then $domain_\bot~f$, $domain \circ g$, and $domain\pre \circ h$ are monotone, decreasing, and idempotent in the subdomains on which they converge.
...
In particular, for any $\meaningofconv{\mathit{p}}\pmap : \set{\pair{}} \pmapto Y$, the probabilities of measurable output sets are well-defined.
...
To interpret programs that may diverge, or that converge with probability $1$, we need to approximate $\convifppre$~\eqref{eqn:ifppre-def}.

Directly implementing $\convifppre$ in terms of $agrees$ cannot work: the preimage of $Bool$ under $agrees$ (as a mapping) is $\set{\pair{true,true},\pair{false,false}}$, which is not rectangular.
...
This does not use $agrees$ and preserves rectangularity if $k_1$, $k_2$ and $k_3$ do.
Unfortunately, if $A_2$ or $A_3$ overapproximates $\emptyset$, it takes unnecessary branches, which can cause divergence.
In the exact semantics, expressions interpreted using $\convifppre$ never diverge.
...
Therefore, if the approximating ${\convifppre}'$ takes \emph{no branches} when $C_b = \set{true,false}$, but approximates with a finite computation, expressions interpreted using ${\convifppre}'$ will always converge.
...
For all $A \in Rect~\pair{\pair{R,T},X}$ and $B \in Rect~Y$, $ap\pre'~(h'~j_0~A)~B$ converges.
\end{corollary}
...
For monotonicity in $A$, let $A' \in Rect~\pair{\pair{R,T},X}$, and $\mathit{e'}$ be a finite expression equivalent to $\mathit{e}$ for $A'$ (Theorem~\ref{thm:equivalent-finite-expression}).
Monotonicity for rectangular subsets of $A'$ follows from structural induction on the interpretation of $\mathit{e'}$.

For monotonicity in $B$, let $\mathit{e'}$ be the finite expression equivalent to $\mathit{e}$ for any fixed $A$.
Monotonicity follows from structural induction on the interpretation of $\mathit{e'}$.
\end{proof}

\begin{theorem}[approximate preimages are decreasing]
...
Let $\mathit{e'}$ be the finite expression equivalent to $\mathit{e}$ for $A$; that approximate preimages are decreasing follows from structural induction on the interpretation of $\mathit{e'}$.
\end{proof}


\begin{corollary}[approximate preimages preserve disjointness]
If $A_1 \i A_2 = \emptyset$, then $ap\pre'~(h'~j_0~A_1)~B \i ap\pre'~(h'~j_0~A_2)~B = \emptyset$.
...
With disjointness and monotonicity properties proved, it is natural to suppose that we can compute probabilities of preimages of $B$ by computing preimages with respect to increasingly fine discretizations of $A$.
...
Theorem~\ref{thm:monotonicity} (monotonicity) guarantees that the union of preimage refinements of a \emph{partition} of $A$ is never larger than the refinement of $A$ itself.
Corollary~\ref{cor:disjointness} (disjointness) guarantees that the refinements of a partition of $A$ are a partition; thus, its measure is the sum of the measures of the rectangles that comprise it.
Theorem~\ref{thm:approximation} guarantees that the algorithm is \keyword{sound}: the true preimage of $B$ is always contained in the covering partition $refine^*$ returns.

We would like it to be \keyword{complete} up to null sets; i.e. covering partitions' measures should converge to the true preimage measure.
...
A counterexample is the expression $rational?~random$, where $rational?$ returns $true$ when its argument is rational and diverges otherwise.
...
XXX: possible citations:
\begin{itemize}
	\item labeling/numbering expressions (check CFA lit.)
	\item bananas and lenses
	\item isomorphic data structure transformations
	\item prolog stuff
	\item constraint-functional programming
\end{itemize}
...
XXX: copy citations below to appropriate places in the introduction

XXX: moar citations

XXX: rework prose below to fit paper better
