...
The restrictions placed on programs are indeed onerous, if benign functions such as $min$ and $max$ must be disallowed.
...
Its absence and the fact that the type parameters denote sets will make it easier to apply well-known theorems from set theory, which know nothing of lambda types and propagating error values.
...
Clearly $\arrmap := \liftmap \circ \arrbot$ meets the first homomorphism identity~\eqref{eqn:lift-distributes-over-arr}.
...
We are not so fortunate: in the limit, we would be computing the \keyword{Jordan outer measure}~(XXX: cite) of the preimage of $B$, which is not always equal to its probability.
...
Unfortunately, the sparsity of work on Jordan measure, especially in general spaces, makes it difficult to characterize the conditions under which a discretization algorithm converges.
...
However, we have found our Typed Racket~(XXX: cite) implementation useful for verifying theorems.
...
We use it to test the semantics agains real-world problems.

%The arrow definitions are similar enough to those given in this paper that we do not need to report on them.
